...
const RegularProjects = dynamic(import('../../src/components/sections/projects/regular'));
...
//     const [gitUserRes, gitReposRes] = await Promise.all([fetch(`https://api.github.com/users/${settings.username.github}`), fetch(`https://api.github.com/users/${settings.username.github}/repos`)]);

//     let [user, repos] = await Promise.all([gitUserRes.json(), gitReposRes.json()]);

//     if (user.login) {
//         user = [user].map(({ login, name, avatar_url, html_url }) => ({ login, name, avatar_url, html_url }));
//     }
